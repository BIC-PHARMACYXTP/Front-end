...
export default function InsuranceOrderRouter() {
  ...
  if (isTravelAccidentInsurance()) return <TravelAccidentInsuranceOrderPage />;
  if (isPersonalAccidentHealth())
    return <PersonalAccidentHealthInsuranceOrderPage />;
  if (isCancerInsurance()) return <CancerInsuranceOrderPage />;
  if (isElectricityAccidentInsurance())
    return <ElectricityAccidentInsuranceOrderPage />;
  if (isExtendedAccidentInsurance())
    return <ExtendedAccidentInsuranceOrderPage />;
  if (isMotorcycleInsurance()) return <MotorcycleCivilLiabilityOrderPage />;
  if (isCarInsurance()) return <CarCivilLiabilityOrderPage />;
  if (isHomeInsurance()) return <HomeInsuranceOrderPage />;

  // Mặc định trả về trang ô tô
  return <CarCivilLiabilityOrderPage />;
}
